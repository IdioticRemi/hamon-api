...
module.exports = class extends Endpoint {
	...
	async run(req, res) {
		...
		let { intensity = 50 } = req.query;

		intensity = parseInt(intensity.toString());

		if (isNaN(intensity) || intensity > 100) return {status: 400, message: "Invalid 'intensity' query field. [0 to 100]"};
		...
		try {
			...
			img.resize(128, 128);
			img.color([{ apply: "brighten", params: [intensity] }]);
